"use strict"
let peopleArr = JSON.stringify([{"id":1,"firstName":"Osmond","surName":"Kieran","sex":"Male","age":26,"isVegetarian":true,"langCount":8},
...
localStorage.setItem('peoples',peopleArr);
const peopleString = localStorage.getItem('peoples');
const peopleArray = JSON.parse(peopleString);
...
let maleCount = 0;
let femaleCount = 0;
peopleArray.forEach((item) => { age += item.age,
    maxLangCount = item.langCount > maxLangCount ? item.langCount : maxLangCount,
    item.sex == "Male" ? maleCount += 1 : femaleCount += 1
});

mid = age/peopleArray.length

alert(`Среднее кол-во людей: ${mid}.
Максимальное кол-во языков: ${maxLangCount}.
Количество девушек: ${femaleCount}.
Количество парней: ${maleCount}.
`)
